/*
 * Hybrid MPI + OpenMP PFC solver.
 *
 * The distributed spectral solver is based on Petri Hirvonen's FFTW-MPI
 * implementation (December 7, 2017). The workflow now supports FFTW threads,
 * OpenMP local loops, multiple input stages, and direct pure-C XYZ output.
 */
...
struct Arrays {
	...
	ptrdiff_t allocation;	// local complex allocation size
...
void configure_arrays(struct Arrays *arrays, FILE *input) {
	...
	arrays->allocation = fftw_mpi_local_size_2d_transposed(arrays->H, arrays->W/2+1, MPI_COMM_WORLD, &arrays->lH, &arrays->lh0, &arrays->lW, &arrays->lw0);
	ptrdiff_t lWHh = arrays->allocation;
	ptrdiff_t lWHp = 2*lWHh;	// local data size padded
	...
	if(arrays->A == NULL || arrays->B == NULL || arrays->p == NULL ||
	   arrays->q == NULL) {
		fprintf(stderr, "Unable to allocate distributed FFTW arrays.\n");
	...
	if(arrays->p_P == NULL || arrays->q_Q == NULL || arrays->Q_q == NULL) {
		fprintf(stderr, "Unable to create distributed FFTW plans.\n");
		MPI_Abort(MPI_COMM_WORLD, 1);
	}
}
...
void configure_output(struct Output *output, FILE *input) {
	...
	if(fscanf(input, " %d %d", &output->T_print, &output->T_write) != 2 ||
	   output->T_print <= 0 || output->T_write <= 0) {
...
void polycrystalline_state(struct Arrays *arrays, double dx, double dy, double l0, double p, double A, int N, double R) {
	...
	double vmin = 0.0;
#pragma omp parallel for schedule(static) private(w,gh,k,n,u,v,r2,G,min,r2min,umin,vmin)
	for(h = 0; h < arrays->lH; h++) {
...
void read_state(struct Arrays *arrays, FILE *file, double p, double A) {
	...
	double global_min;
	double global_max;
	MPI_Allreduce(&p0, &global_p0, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(&Amin, &global_min, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
	MPI_Allreduce(&Amax, &global_max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
	p0 = global_p0/((double)arrays->W*arrays->H);
	Amin = global_min;
	Amax = global_max;
	if(Amax == Amin) {
		fprintf(stderr, "Cannot normalize a constant input state.\n");
		MPI_Abort(MPI_COMM_WORLD, 3);
	}
#pragma omp parallel for schedule(static) private(w,k)
	for(h = 0; h < arrays->lH; h++) {
		k = Wp*h;
		for(w = 0; w < arrays->W; w++) {
...
void update_AB(struct Arrays *arrays, struct Model *model, struct Relaxation *relaxation) {
	...
	double c = model->c;
#pragma omp parallel for schedule(static) private(h,gw,k,ky,kx2,k2,d2,l,expl)
...
void fp(struct Arrays *arrays, struct Model *model, struct Relaxation *relaxation) {
	...
	int w, h, gw, k;
#pragma omp parallel for schedule(static) private(h,gw,k,kx2,ky,k2,d2)
	for(w = 0; w < arrays->lW; w++) {
		gw = arrays->lw0+w;
		kx2 = gw*dkx;
		kx2 *= kx2;
...
#pragma omp parallel for schedule(static) private(w,k,p,q,p2) reduction(+:local_f,local_p)
	for(h = 0; h < arrays->lH; h++) {
		k = Wp*h;
		for(w = 0; w < arrays->W; w++) {
			p = arrays->p[k+w];
			...
			local_f += 0.5*( alpha*p2 + beta*p*q ) + div3*gamma*p*p2 + 0.25*delta*p2*p2;
			// restore q arrays
			arrays->q[k+w] = p;
			// average densities
			local_p += p;
	...
	MPI_Allreduce(&local_f, &relaxation->f, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(&local_p, &relaxation->p, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	double divWH = 1.0/arrays->W/arrays->H;
	relaxation->f *= divWH;
	relaxation->p *= divWH;
...
void step(struct Arrays *arrays, struct Model *model, struct Relaxation *relaxation) {
	...
	double gamma = model->gamma;
	double delta = model->delta;
#pragma omp parallel for schedule(static) private(w,k,q,q2)
	for(h = 0; h < arrays->lH; h++) {
		k = Wp*h;
		for(w = 0; w < arrays->W; w++) {
			q = arrays->q[k+w];
	...
	fftw_complex *Q = (fftw_complex*)&arrays->q[0];
#pragma omp parallel for schedule(static) private(h,k)
	for(w = 0; w < arrays->lW; w++) {
		k = arrays->H*w;
		for(h = 0; h < arrays->H; h++) {
...
void clear_arrays(struct Arrays *arrays) {
	fftw_destroy_plan(arrays->p_P);
	fftw_destroy_plan(arrays->q_Q);
	fftw_destroy_plan(arrays->Q_q);
	fftw_free(arrays->p);
	fftw_free(arrays->q);
	fftw_free(arrays->A);
...
static void run_input(const char *name, struct Arrays *arrays,
	...
	if(input == NULL) {
		if(rank == 0) fprintf(stderr, "Input file not found: %s\n", filename);
	...
	while(fscanf(input, " %c", &label) != EOF) {
		if(label == '#' && fgets(line, sizeof(line), input)) {}
		else if(label == 'S') seed_rngs(relaxation, input);
		else if(label == 'O') configure_output(&output, input);
		else if(label == 'A') configure_arrays(arrays, input);
		else if(label == 'I') initialize_system(arrays, input);
		else if(label == 'M') configure_model(&model, input);
		else if(label == 'R') {
			...
			if(rank == 0) fprintf(stderr, "Invalid input label: %c\n", label);
			MPI_Abort(MPI_COMM_WORLD, 1);
		}
	}
	fclose(input);
...
static void gather_and_write_xyz(const struct Arrays *arrays,
	...
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	MPI_Comm_size(MPI_COMM_WORLD, &size);
	ptrdiff_t local_count_pd = arrays->lH * arrays->W;
	...
	int local_count = (int)local_count_pd;
	int stride = 2*(arrays->W/2+1);
...
#pragma omp parallel for schedule(static)
	for(ptrdiff_t h = 0; h < arrays->lH; h++) {
		memcpy(packed + h*arrays->W, arrays->q + h*stride,
			   (size_t)arrays->W*sizeof(*packed));
	}

	int *counts = NULL;
	int *displacements = NULL;
	long long *starts = NULL;
	double *global = NULL;
	long long local_start = (long long)arrays->lh0 * arrays->W;
	if(rank == 0) {
		counts = malloc((size_t)size*sizeof(*counts));
		displacements = malloc((size_t)size*sizeof(*displacements));
		starts = malloc((size_t)size*sizeof(*starts));
	...
	MPI_Gather(&local_count, 1, MPI_INT, counts, 1, MPI_INT, 0,
			   MPI_COMM_WORLD);
	MPI_Gather(&local_start, 1, MPI_LONG_LONG, starts, 1, MPI_LONG_LONG, 0,
			   MPI_COMM_WORLD);
	if(rank == 0) {
		for(int i = 0; i < size; i++) {
			if(starts[i] < 0 || starts[i] > INT_MAX ||
			   starts[i] + counts[i] > global_count_pd) {
				fprintf(stderr, "Invalid distributed slab layout at rank %d.\n",
						i);
				MPI_Abort(MPI_COMM_WORLD, 4);
			}
	...
	MPI_Gatherv(packed, local_count, MPI_DOUBLE, global, counts,
...
int main(int argc, char **argv) {
	...
	for(int i = 1; i < argc; i++) {
		...
		else if(strcmp(argv[i], "--angstrom-lattice") == 0 && i+1 < argc) {
	...
	if(!fftw_init_threads()) {
		if(rank == 0) fprintf(stderr, "Unable to initialize FFTW threads.\n");
		MPI_Abort(MPI_COMM_WORLD, 1);
	}
	...
	if(rank == 0) {
		int size;
		MPI_Comm_size(MPI_COMM_WORLD, &size);
		printf("Using %d MPI ranks x %d OpenMP/FFTW threads\n", size,
			   threads);
	}

	struct Arrays arrays = {0};
	struct Relaxation relaxation = {0};
